...
def load_image(url, dtype = 'float'):
    """
    Load image from a given url path
    Inputs:
        imPath= image url/ path location
    Outputs:
        img= array containing image
    """
    ...
    if dtype == 'float':
        image = np.float32(image.astype(float)/255)
